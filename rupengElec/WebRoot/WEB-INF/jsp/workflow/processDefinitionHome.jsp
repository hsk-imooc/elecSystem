	...
	<div style="width:90%;background-color: #f5fafe;">
		...
		<div>
			...
			<div align="right" style="float: right;">
			    ...
			    <input  type="button" value="部署流程定义文档"  onclick="openWindow('${pageContext.request.contextPath }/workflow/processDefinitionAction_addPage.action','700','400')" />
			</div>
		...
		<table id="table"  style="width: 100%" cellspacing="0"  border="0">
			<thead style="height: 20px;">
					...
					<s:iterator value="%{#request.processDefinitions}" var="pd">
						<tr>
							<td><s:property value="%{#pd.name}"/></td>
							<td><s:property value="%{#pd.version}"/></td>
							<td><s:property value="%{#pd.description}"/></td>
							<td>
								<a href="javascript:void(0)" onclick="openWindow(' ${pageContext.request.contextPath}/workflow/processDefinitionAction_showProcessImage.action?id=<s:property value="%{#pd.id}"/>',800,500);">查看流程图</a>
								<a href="${pageContext.request.contextPath}/workflow/processDefinitionAction_delete.action?id=<s:property value="%{#pd.id}"/>" onclick="return confirm('你确定要删除 员工请假申请流程？')">
